...
/**
 * Ingestion-side orchestration: turns raw AIS stream payloads into repository
 * writes. Lives in the ingestion worker process only (see ingestion-app.module.ts).
 */
@Injectable()
export class ShipIngestionService {
  private readonly logger = new Logger(ShipIngestionService.name);
